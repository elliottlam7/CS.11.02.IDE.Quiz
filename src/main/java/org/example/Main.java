...
public class Main {
    ...
    public static String twentyFourToTwelve(String time) {
        ...
        int hourNumber = Integer.parseInt(substring1);
        int hourTwelve = hourNumber % 12;
        return hourTwelve + ":" + substring2 + " pm";
    }
    ...
    public static double distanceBetweenTwoPoints(int x1, int y1, int x2, int y2) {
        double difference = (Math.round(Math.sqrt(Math.pow((x2 - x1),2) + Math.pow((y2 - y1),2)) * 100.0)) / 100.0;
        return difference;
    }
